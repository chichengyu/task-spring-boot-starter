...
```
如果只是使用` 2.0.0.RELEASE ` 版本的 ` task定时任务`，可以参考下面导包坐标(排除多余依赖)，也可以直接使用之前版本[1.3.3.RELEASE](https://github.com/chichengyu/task-spring-boot-starter)(`只包含定时任务依赖`)，虽然不排除也没什么影响，但可以使项目体量小一些，导包
```
<dependency>
    <groupId>io.github.chichengyu</groupId>
    <artifactId>task-spring-boot-starter</artifactId>
